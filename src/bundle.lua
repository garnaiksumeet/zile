...
local function load_bundle (filename)
  ...
  res, errmsg = io.slurp (filename), "file not found"
  if res then
...
Defun ("set_theme",
       {"string"},
[[
Set the current display attributes according to @i{theme_name}.
]],
  true,
  function (theme_name)
    theme_name = theme_name or minibuf_read ("Theme name: ", "")

    local t = load_bundle (PATH_THEMEDIR .. "/" .. theme_name .. ".lua")
