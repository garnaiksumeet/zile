...
local options = {
  ...
  {"opt", "funcall", 'f', "required", "FUNC", "call " .. PROGRAM_NAME .. " Lua function FUNC with no arguments"},
  {"opt", "load", 'l', "required", "FILE", "load " .. PROGRAM_NAME .. " Lua FILE using the load function"},
...
function main ()
  ...
  for i = 1, #zarg do
    ...
    elseif type == "file" then
      ok = find_file (arg)
      if ok then
        eval.execute_function ("goto_line", line)
      end
    end
  ...
  if #buffers == 3 then
    ...
    eval.execute_function ("other_window")
  elseif #buffers > 3 then
    eval.execute_function ("list_buffers")
  end
